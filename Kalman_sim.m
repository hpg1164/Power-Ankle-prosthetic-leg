...
for i = 1:length(time_vector)
    move_control_vector(2,i) = T_amb;
    modulus = mod(time_vector(i),move_period);
    if (modulus/move_period <move_duty_cycle)
       move_control_vector(1,i) = I_move^2;
    else
       move_control_vector(1,i) = 0;
    end
end
% 
% figure;
% plot(time_vector,move_control_vector(1,:).^0.5,'k.-')
% xlabel('time (s)')
% ylabel('current (A)')
% title(['Current profile:moveing with period ',num2str(move_period),...
%     ' s and duty cycle ',num2str(move_duty_cycle),'.']);
...
ICs = [T_amb; T_amb; T_amb];
[T, output, T_noisy, output_noisy, kalman_estimate] = simulator(ICs,...
    time_vector,move_control_vector,params,...
    internal_Kalman_option);
...
xlabel('time (s)')
ylabel('Temperature (deg C)')
title({['Simulated temperature profile:moveing (I = ',num2str(I_move),...
    ' A)'],[' with period ',num2str(move_period),...
    ' s and duty cycle ',num2str(move_duty_cycle),'.']});
hold off
...
xlabel('time(s)')
ylabel('Temperature (deg C)')
title('Measurement vs Kalman filter estimate (moving)')
